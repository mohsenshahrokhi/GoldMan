...
class OrderExecutor:
    ...
    def execute_order(self, signal: OrderSignal) -> Optional[int]:
        ...
        try:
            ...
            if signal.direction == "BUY":
                if sl >= price:
                    logger.error(f"[SENSITIVE] Invalid SL for BUY: SL={sl:.5f} must be below entry={price:.5f}")
                    return None
                if tp <= price:
                    logger.error(f"[SENSITIVE] Invalid TP for BUY: TP={tp:.5f} must be above entry={price:.5f}")
                    return None
            else:  # SELL
                if sl <= price:
                    logger.error(f"[SENSITIVE] Invalid SL for SELL: SL={sl:.5f} must be above entry={price:.5f}")
                    return None
                if tp >= price:
                    logger.error(f"[SENSITIVE] Invalid TP for SELL: TP={tp:.5f} must be below entry={price:.5f}")
                    return None
            
            symbol_info = mt5.symbol_info(signal.symbol)
            if symbol_info:
                min_distance = symbol_info.point * 10  # حداقل 10 پیپ
                
                if signal.direction == "BUY":
                    if abs(price - sl) < min_distance:
                        logger.warning(f"[SENSITIVE] SL too close to entry: Distance={abs(price - sl):.5f}, Min={min_distance:.5f}")
                        sl = price - min_distance * 2
                        logger.info(f"[SENSITIVE] Auto-adjusted SL to: {sl:.5f}")
                    
                    if abs(tp - price) < min_distance:
                        logger.warning(f"[SENSITIVE] TP too close to entry: Distance={abs(tp - price):.5f}, Min={min_distance:.5f}")
                        tp = price + min_distance * 2
                        logger.info(f"[SENSITIVE] Auto-adjusted TP to: {tp:.5f}")
                else:  # SELL
                    if abs(sl - price) < min_distance:
                        logger.warning(f"[SENSITIVE] SL too close to entry: Distance={abs(sl - price):.5f}, Min={min_distance:.5f}")
                        sl = price + min_distance * 2
                        logger.info(f"[SENSITIVE] Auto-adjusted SL to: {sl:.5f}")
                    
                    if abs(price - tp) < min_distance:
                        logger.warning(f"[SENSITIVE] TP too close to entry: Distance={abs(price - tp):.5f}, Min={min_distance:.5f}")
                        tp = price - min_distance * 2
                        logger.info(f"[SENSITIVE] Auto-adjusted TP to: {tp:.5f}")
            ...
            margin_free = getattr(account_info, 'margin_free', getattr(account_info, 'free_margin', 0.0))
    ...
    def pre_order_validation(self, signal: OrderSignal) -> bool:
        ...
        if hasattr(account_info, 'margin_free'):
            free_margin = account_info.margin_free
        elif hasattr(account_info, 'free_margin'):
            free_margin = account_info.free_margin
        else:
            logger.error("Cannot determine free margin")
            return False
        ...
        if required_margin > free_margin:
            logger.error(f"Insufficient margin: Required={required_margin:.2f}, Available={free_margin:.2f}")
            return False
    ...
    def manage_position(self, ticket: int, strategy_manager: StrategyManager, 
        ...
        try:
            ...
            if profit_pct >= 80:
                logger.info(f"[SENSITIVE] Position reached 80% profit: Ticket={ticket}, Symbol={symbol}, Profit={profit_pct:.2f}% - Applying maximum trailing stop only")
    ...
    def apply_trailing_stop(self, position: Any, profit_pct: float, 
        ...
        min_distance = 5 * symbol_info.point  # حداقل 5 پیپ
        adjustment = max(adjustment, min_distance)
                ...
                if direction == "BUY":
                    ...
                    if current_sl > 0:
                        if new_sl <= current_sl:
                            new_sl = current_sl + min_distance
                        elif new_sl < current_sl + min_distance:
                            new_sl = current_sl + min_distance
                else:
                    new_sl = current_price + (1.5 * adjustment)
                    if current_sl > 0:
    ...
    def apply_partial_exit(self, position: Any, profit_pct: float, risk_manager: RiskManager, strategy: Any = None):
        ...
        symbol_info = mt5.symbol_info(position.symbol) if mt5 else None
        if symbol_info:
            logger.info(f"[PARTIAL_EXIT] Symbol info: {position.symbol}, FillingMode={symbol_info.filling_mode if hasattr(symbol_info, 'filling_mode') else 'N/A'}")
    ...
    def update_stop_loss(self, ticket: int, new_sl: float) -> bool:
        ...
        try:
            ...
            tolerance = symbol_info.point * 20 if symbol_info else 0.0001
            
            if abs(position.tp - original_tp) > tolerance:
                logger.warning(f"[SENSITIVE] TP changed by user detected: Ticket={ticket}, OriginalTP={original_tp:.5f}, CurrentTP={position.tp:.5f}, Diff={abs(position.tp - original_tp):.5f}, Tolerance={tolerance:.5f}. Restoring original TP.")
            
            current_price = position.price_current
            if position.type == mt5.ORDER_TYPE_BUY:
                if new_sl >= current_price:
                    logger.error(f"[SENSITIVE] Invalid SL for BUY: NewSL={new_sl:.5f} must be below current price={current_price:.5f}")
                    min_distance = symbol_info.point * 10 if symbol_info else 0.001
                    new_sl = current_price - min_distance
                    logger.info(f"[SENSITIVE] Auto-adjusted SL to: {new_sl:.5f}")
            else:  # SELL
                if new_sl <= current_price:
                    logger.error(f"[SENSITIVE] Invalid SL for SELL: NewSL={new_sl:.5f} must be above current price={current_price:.5f}")
                    min_distance = symbol_info.point * 10 if symbol_info else 0.001
                    new_sl = current_price + min_distance
                    logger.info(f"[SENSITIVE] Auto-adjusted SL to: {new_sl:.5f}")
...
• TP: {old_tp:.5f}"""
    ...
    def close_partial_position(self, ticket: int, volume: float) -> bool:
        ...
        try:
            ...
            min_volume = 0.01  # حداقل حجم قابل معامله
            
            if volume < min_volume:
                logger.warning(f"[PARTIAL_EXIT] Volume too small: {volume:.2f} < {min_volume:.2f}, skipping partial exit")
                return False
            
            volume = round(volume, 2)
            
            if volume >= position.volume:
                logger.info(f"[PARTIAL_EXIT] Closing entire position instead of partial")
            ...
            logger.info(f"[PARTIAL_EXIT] Filling modes to try: {modes_to_try} (symbol_filling={symbol_info.filling_mode})")
            ...
            for filling_mode in modes_to_try:
                ...
                result = mt5.order_send(request)
                if result.retcode == mt5.TRADE_RETCODE_DONE:
                    logger.info(f"[PARTIAL_EXIT_SUCCESS] Closed partial with filling mode {filling_mode}")
                    break
                elif result.retcode == 10030:  # Unsupported filling mode
                    logger.warning(f"[PARTIAL_EXIT_RETRY] Filling mode {filling_mode} not supported, trying next mode...")
                    continue
                else:
                    logger.warning(f"[PARTIAL_EXIT_RETRY] Filling mode {filling_mode} failed: {result.comment}")
                    continue
            
            if result is None or result.retcode != mt5.TRADE_RETCODE_DONE:
                logger.error(f"[SENSITIVE] Failed partial exit after trying all filling modes: Ticket={ticket}, Retcode={result.retcode if result else 'None'}, Comment={result.comment if result else 'No result'}")
                return False
            ...
            logger.info(f"[SENSITIVE] Partial exit successful: Ticket={ticket}, Symbol={position.symbol}, ClosedVolume={volume:.2f}, RemainingVolume={remaining_volume:.2f}, Balance={balance:.2f}, Equity={equity:.2f}")
...
• Equity: ${equity:.2f}"""
    ...
    def close_position(self, ticket: int) -> bool:
        ...
        try:
            ...
            available_modes = self.get_available_filling_modes(symbol_info)
            all_modes = [mt5.ORDER_FILLING_FOK, mt5.ORDER_FILLING_RETURN, mt5.ORDER_FILLING_IOC]
            modes_to_try = available_modes + [m for m in all_modes if m not in available_modes]
            ...
            for filling_mode in modes_to_try:
                ...
                result = mt5.order_send(request)
                if result.retcode == mt5.TRADE_RETCODE_DONE:
                    logger.info(f"[CLOSE_POSITION] Closed successfully with filling mode {filling_mode}")
                    break
                elif result.retcode == 10030:
                    logger.warning(f"[CLOSE_POSITION] Filling mode {filling_mode} not supported, trying next mode...")
                    continue
                else:
                    logger.warning(f"[CLOSE_POSITION] Filling mode {filling_mode} failed: {result.comment}")
                    continue
            
            if result is None or result.retcode != mt5.TRADE_RETCODE_DONE:
                logger.error(f"[SENSITIVE] Failed to close position after trying all filling modes: Ticket={ticket}, Retcode={result.retcode if result else 'None'}, Comment={result.comment if result else 'No result'}")
                return False
            ...
            if account_info:
                balance = getattr(account_info, 'balance', 0.0)
                equity = getattr(account_info, 'equity', 0.0)
                logger.info(f"[SENSITIVE] Position closed successfully: Ticket={ticket}, Symbol={symbol}, Volume={volume:.2f}, Balance={balance:.2f}, Equity={equity:.2f}")
            else:
