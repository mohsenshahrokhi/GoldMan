...
class RLEngine:
    ...
    def __init__(self, db_manager: DatabaseManager):
        ...
        self.default_weights = {
            ...
            'garch': 0.20,
            'fixed_rr': 0.20,
            'technical': 0.20  # وزن اندیکاتورهای تکنیکال
        }
        ...
        self.parameter_constraints = {
            ...
            'min_rr_ratio': (1.0, 2.5),  # Lower bound changed to 1.0 for testing. Original: 1.2
    ...
    def calculate_reward(self, order_profit: float, transaction_cost: float,
                        ...
                        close_reason: str = "UNKNOWN", trailing_stop_applied: bool = False,
                        technical_signal: str = None) -> float:
        if close_reason == "TP":
        ...
        technical_bonus = 0.0
        if technical_signal and order_profit > 0:
            if 'STRONG' in technical_signal:
                technical_bonus = order_profit * 0.03  # پاداش سیگنال قوی صحیح
            elif 'WEAK' in technical_signal:
                technical_bonus = order_profit * 0.01  # پاداش سیگنال ضعیف صحیح
        elif technical_signal and order_profit < 0:
            if 'STRONG' in technical_signal:
                technical_bonus = abs(order_profit) * -0.05  # جریمه سیگنال قوی اشتباه
            elif 'NEUTRAL' in technical_signal:
                technical_bonus = abs(order_profit) * -0.02  # جریمه سیگنال خنثی اشتباه

        total_reward = base_reward + rr_bonus - time_penalty - drawdown_penalty + max_profit_bonus + technical_bonus
    ...
    def optimize_based_on_result(self, symbol: str, strategy: str, order_id: int,
                                 ...
                                 max_profit: float, order_profit: float, technical_signal: str = None):
        if self.should_optimize(symbol, strategy):
            ...
            if technical_signal:
                self.optimize_technical_weights(symbol, strategy, technical_signal, order_profit)

    def optimize_technical_weights(self, symbol: str, strategy: str, technical_signal: str, profit: float):
        """
        بهینه‌سازی وزن‌های اندیکاتورهای تکنیکال بر اساس عملکرد

        پارامترها:
        - technical_signal: سیگنال ترکیبی (STRONG_BULLISH, WEAK_BEARISH, etc.)
        - profit: سود/زیان معامله
        """
        cursor = self.db.conn.cursor()

        current_weights = self.get_weights(symbol, strategy)
        technical_weight = current_weights.get('technical', 0.2)

        if profit > 0:  # معامله سودده
            if 'STRONG' in technical_signal:
                technical_weight = min(technical_weight + 0.05, 0.5)
            elif 'WEAK' in technical_signal:
                technical_weight = min(technical_weight + 0.02, 0.4)
        else:  # معامله زیان‌ده
            if 'STRONG' in technical_signal:
                technical_weight = max(technical_weight - 0.03, 0.1)
            elif 'NEUTRAL' in technical_signal:
                technical_weight = max(technical_weight - 0.01, 0.15)

        total_weight = sum([current_weights.get(k, 0) for k in ['node', 'atr', 'garch', 'fixed_rr']])
        if total_weight > 0:
            scale_factor = (1 - technical_weight) / total_weight
            for key in ['node', 'atr', 'garch', 'fixed_rr']:
                current_weights[key] *= scale_factor

        current_weights['technical'] = technical_weight

        self.save_weights(symbol, strategy, current_weights)
        logger.info(f"[RL] Technical weights optimized: {current_weights}")

    def save_weights(self, symbol: str, strategy: str, weights: Dict[str, float]):
        """
        ذخیره وزن‌های بهینه‌سازی شده در دیتابیس
        """
        cursor = self.db.conn.cursor()

        cursor.execute("""
            DELETE FROM rl_parameters
            WHERE symbol = ? AND strategy = ? AND parameter_name LIKE 'weight_%'
        """, (symbol, strategy))

        for key, value in weights.items():
            param_name = f'weight_{key}'
            cursor.execute("""
                INSERT OR REPLACE INTO rl_parameters
                (symbol, strategy, parameter_name, parameter_value, updated_at)
                VALUES (?, ?, ?, ?, datetime('now'))
            """, (symbol, strategy, param_name, value))
